...
"""Service client for interacting with the C4H API."""
...
class C4HService:
    ...
    async def submit_job(self, work_order: WorkOrder) -> JobSubmissionResponse:
        """Submit a work order to the C4H service."""
        if not self.api_key:
            raise ValueError(f"C4H API key not found in environment variable {self.api_key_env}")
        
        # Get submission payload
        payload = work_order.to_submission_payload()
        
        # Add service config if specified
        if work_order.config and work_order.config.service_id:
            payload["service_id"] = work_order.config.service_id
        ...
        # Log submission
        logger.info(f"Submitting work order {work_order.id} to C4H service")

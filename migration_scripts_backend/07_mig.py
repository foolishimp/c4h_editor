#!/usr/bin/env python
# Script to create generic API routes for configurations
...
# Create configs.py
configs_route_path = routes_dir / "configs.py"
with open(configs_route_path, "w") as f:
    f.write("""# backend/api/routes/configs.py
\"\"\"
...
async def create_config(
    ...
    lineage_tracker: Optional[LineageTracker] = Depends(get_lineage_tracker)
):
    ...
    try:
        ...
        if lineage_tracker:
            lineage_tracker.track_workorder_creation(
                workorder_id=config.id,
                version=config.metadata.version,
                commit_hash=commit,
                metadata=config.metadata.dict()
            )
...
async def update_config(
    ...
    try:
        ...
        # Track lineage if available
        if lineage_tracker:
            lineage_tracker.track_workorder_update(
                workorder_id=updated_config.id,
                version=updated_config.metadata.version,
                commit_hash=commit,
                metadata=updated_config.metadata.dict()
            )
...
async def delete_config(
    ...
    try:
        ...
        # Track lineage if available
        if lineage_tracker:
            lineage_tracker.track_workorder_deletion(
                workorder_id=config_id,
                commit_hash=commit
            )
...
print(f"Created {configs_route_path}")

print("Generic API Routes for Configurations created successfully.")